...
type Item = { productId?: number; name: string; qty: number; unit?: string; unitCost?: number };

export default function PembelianLangsungAddPage() {
  const router = useRouter();
  const [purchaseNumber, setPurchaseNumber] = useState("");
  const [date, setDate] = useState<string>(new Date().toISOString().slice(0,10));
  const [supplierName, setSupplierName] = useState("Shopee");
  const [marketplaceOrderId, setMarketplaceOrderId] = useState("");
  const [notes, setNotes] = useState("");
  const [items, setItems] = useState<Item[]>([{ name: "", qty: 1, unit: "pcs" }]);
  ...
  useEffect(() => {
    (async () => {
      const res = await fetch(`/api/purchases/direct/next-number?date=${date}`);
      const json = await res.json();
      setPurchaseNumber(json.number || "");
    })();
  }, [date]);
  ...
  const save = async () => {
    ...
    try {
      ...
      fd.set("items", JSON.stringify(items.map(i => ({...i, qty: Number(i.qty||0), unitCost: Number(i.unitCost||0)}))));
      const files = fileRef.current?.files;
      if (files) {
        for (let i=0;i<files.length;i++) fd.append("attachments", files[i]);
      }
      const res = await fetch(`/api/purchases/direct`, { method: "POST", body: fd });
      const json = await res.json();
      if (json.success) {
        router.push(`/pembelian/pembelian-langsung/${json.data.id}`);
      } else {
  ...
  return (
    <div className="p-4 space-y-4">
      <h1 className="text-xl font-semibold">Tambah Pembelian Langsung</h1>
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <div className="md:col-span-2">
          <label className="block text-sm">Lampiran (nota/resi)</label>
