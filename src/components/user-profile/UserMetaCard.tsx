  ...
  const handleAvatarChange = async (file: File | null) => {
    try {
      ...
      if (file.size > 5 * 1024 * 1024) {
        setAvatarError("Ukuran file terlalu besar. Maksimum 5MB");
        return;
      }
      // Buka modal cropper dengan sumber gambar dari file
      const reader = new FileReader();
      reader.onload = () => {
        setCropSource(reader.result as string);
        setCropperOpen(true);
      };
      reader.onerror = () => setAvatarError("Gagal membaca file gambar");
      reader.readAsDataURL(file);
    } catch (e: any) {
      setAvatarError(e?.message || "Gagal memproses avatar");
    }
  };

  const handleApplyCropped = async (blob: Blob) => {
    try {
      setAvatarError(null);
        ...
        <div className="flex flex-col gap-5 xl:flex-row xl:items-center xl:justify-between">
          <div className="flex flex-col items-center w-full gap-6 xl:flex-row">
            ...
            <div className="flex items-center order-2 gap-2 grow xl:order-3 xl:justify-end">
              <a        
        target="_blank"
        rel="noreferrer" href={profile?.facebook || "#"} className="flex h-11 w-11 items-center justify-center gap-2 rounded-full border border-gray-300 bg-white text-sm font-medium text-gray-700 shadow-theme-xs hover:bg-gray-50 hover:text-gray-800 dark:border-gray-700 dark:bg-gray-800 dark:text-gray-400 dark:hover:bg-white/[0.03] dark:hover:text-gray-200">
                <svg
            ...
            <div className="custom-scrollbar h-[450px] overflow-y-auto px-2 pb-3">
              <div>
                ...
                <div className="grid grid-cols-1 gap-x-6 gap-y-5 lg:grid-cols-2">
                  ...
                  <div>
                    <Label>X.com</Label>
                    <Input type="text" value={formData.twitter || ""} onChange={(e) => onChange("twitter", e.target.value)} />
                  </div>
              ...
              <div className="mt-7">
                ...
                <div className="grid grid-cols-1 gap-x-6 gap-y-5 lg:grid-cols-2">
                  ...
                  <div className="col-span-2 lg:col-span-1">
                    ...
                    <Input type="text" value={formData.email || ""} onChange={(e) => onChange("email", e.target.value)} />
                  </div>

                  <div className="col-span-2 lg:col-span-1">
                    <Label>Phone</Label>
                    <Input type="text" value={formData.phone || ""} onChange={(e) => onChange("phone", e.target.value)} />
                  </div>

                  <div className="col-span-2">
                    <Label>Bio</Label>
                    <Input type="text" value={formData.bio || ""} onChange={(e) => onChange("bio", e.target.value)} />
                  </div>
                </div>
              </div>
            </div>
